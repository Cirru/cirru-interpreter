...
var EventEmitter, a_token, an_expression, assert, be_type, check_numbers, cirru_error, cirru_name, cirru_read, cirru_variable, error, fs, has_no_undefined, length_equal, longer_than, main, ms, parse, path, print, reloading, stringify, type, util, watch_scope, _ref, _ref1;
...
cirru_variable = function(scope, x) {
  if (a_token(x)) {
    return scope[x.text];
  } else if (an_expression(x)) {
    return main.interpret(scope, x);
  } else {
    return cirru_error(x, "variable " + x + " not recognized");
  }
};

cirru_name = function(scope, x) {
  if (a_token(x)) {
    return x.text;
  } else if (an_expression(x)) {
    return main.interpret(scope, x);
  } else {
    return cirru_error(x, "name " + x + " not recognized");
  }
};
...
exports.prelude = {
  ...
  "eval": function(scope, list) {
    ...
    assert((code.parent != null) && (code.ast != null), 'should be code');
    child = {
      parent: code.parent,
      outer: scope
    };
  ...
  call: function(scope, list) {
    ...
    code = cirru_variable(scope, args[0]);
    assert((code.parent != null) && (code.ast != null), 'should be code');
    args.slice(1).map(a_token);
    child = {
      parent: code.parent,
      outer: scope,
      args: args.slice(1).map(function(x) {
        return x.text;
      })
    };
    ret = null;
